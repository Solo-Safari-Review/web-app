<div class="hidden" id="confirmDeleteReview">
    <div class="z-50 flex flex-col min-w-[350px] max-w-[600px] h-[400px] mx-auto gap-6 px-8 xl:px-[72px] py-16 fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#D9D9D9] rounded-2xl items-center align-top">
        ...
        <div class="w-full flex flex-col gap-8 text-center">
            <span class="font-extrabold">Hapus Review</span>
            <p class="font-medium">Apakah anda yakin untuk menghapus review ini? Review yang telah dihapus akan tersimpan dalam sampah</p>
            <div class="flex justify-between">
                <button type="button" class="button bg-[#2C2C2C] px-4 py-2 rounded-lg text-white" onclick="$('#confirmDeleteReview').hide()">Batalkan</button>
                <button type="submit" id="deleteReviewButton" class="bg-[#4E1F00] px-4 py-2 rounded-lg text-white">Hapus</button>
            </div>
        </div>
    </div>
</div>
...
@push('scripts')
<script>
function confirmDeleteReview(form) {
    $("#confirmDeleteReview").show();
    $("#deleteReviewButton").attr('form', form);
}
</script>
@endpush
