        ...
        <div class="flex flex-col w-full gap-4 py-1">
            <div class="flex flex-wrap xl:flex-nowrap w-full gap-8 py-2.5">
                <div class="flex flex-col w-full gap-4">
                    ...
                    <select name="category" id="category" class="bg-[#D9D9D9] rounded-2xl px-4 py-2 border-0 focus:border-[#907B60] focus:ring-[#907B60]">
            ...
            <div class="flex flex-wrap xl:flex-nowrap w-full gap-8 py-2.5">
                ...
                <div class="flex flex-col w-full gap-4">
                    ...
                    <div class="flex gap-4 flex-wrap xl:flex-nowrap w-full">
                        <select name="status[review]" id="reviewStatus" class="bg-[#D9D9D9] rounded-2xl px-4 py-2 border-0 focus:border-[#907B60] focus:ring-[#907B60] w-full">
                        ...
                        <select name="status[answer]" id="answerStatus" class="bg-[#D9D9D9] rounded-2xl px-4 py-2 border-0 focus:border-[#907B60] focus:ring-[#907B60] w-full">
            ...
            <div class="flex flex-wrap xl:flex-nowrap w-full gap-8 py-2.5">
                <div class="flex flex-col w-full gap-4">
                    ...
                    <textarea rows="2" name="comment[review_admin]" id="adminReviewComment" class="bg-[#D9D9D9] rounded-2xl px-4 py-2 border-0 focus:border-[#907B60] focus:ring-[#907B60]">{{ $review->categorizedReview->review_admin_comment ?? 'Belum dikomentari' }}</textarea>
                </div>
                <div class="flex flex-col w-full gap-4">
                    <label for="adminDepartmentComment" class="xl:text-lg text-sm font-bold">Komentar Admin Departemen</label>
                    <textarea rows="2" name="comment[department_admin]" id="adminDepartmentComment" class="bg-[#D9D9D9] rounded-2xl px-4 py-2 border-0 focus:border-[#907B60] focus:ring-[#907B60]" disabled>{{ $review->categorizedReview->department_admin_comment ?? 'Belum dikomentari' }}</textarea>
                </div>
            </div>
            <div class="flex flex-col w-full gap-4">
            ...
            <div class="flex w-full gap-4 items-center">
                <label class="xl:text-lg text-sm font-bold">Topik: </label>
                <select id="topics" class=" w-full" name="topics[]" multiple="multiple">
                    @foreach ($topics as $topic)
                        <option value="{{ $topic->id }}" {{ $review->topics->contains($topic->id) ? 'selected' : '' }}>{{ $topic->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
